...
function Profile() {
  ...
  const handleInputChange = (e) => {
    ...
    const newData = type === 'file' ? { [name]: files[0] } : { [name]: value };
    setEditedUserData(prevData => ({
        ...prevData,
        ...newData
    }));
  ...
  const handleSaveUserDetails = () => {
    ...
    axios.put(`${BASE_URl}/api/users/updateUser-personal/${userData.id}`, formData)
        .then(response => {
  ...
  const handleSaveProfessionalDetails = () => {
    axios
      .put(
        `${BASE_URl}/api/users/updateUser-professional/${userData.id}`,
        editedProfessionalData
      )
      .then((response) => {
        console.log("User details updated successfully:", response.data);
        Swal.fire({
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BASE_URl}/api/nominees/getNomineesForOwner/${userData.userName}`)
      .then((response) => {
  ...
  return (
    <div>
      ...
      <LogoutIcon
        variant="primary"
        style={{ marginLeft: "750px", fontSize: "36px" }}
        onClick={handleLogout}
      ></LogoutIcon>
       {editedUserData.image && (
            <img src={editedUserData.image} alt="Profile" style={{ maxWidth: '100%', maxHeight: '100%' }} />
        )}
      ...
      <table class="table table-striped mt-3" style={{ borderRadius: "10px" }}>
        ...
        <tbody>
          <tr className="border border-dark">
            <th scope="row" className="border border-dark">
              User Name
            </th>
        ...
        <tbody>
          ...
          <tr className="border border-dark">
            ...
            <td className="border border-dark">
              {renderPrice(userData.ctc)} {currency}
            </td>
          ...
          <tr className="border border-dark">
            ...
            <td className="border border-dark">
              {renderPrice(userData.pfTotalPaid)} {currency}
            </td>
            <th className="border border-dark"> Monthly EMI</th>
            <td className="border border-dark">
              {renderPrice(userData.monthlyEMI)}
              {currency}
            </td>
          </tr>
          <tr className="border border-dark">
            ...
            <td className="border border-dark">
              {renderPrice(userData.yearlyBonus)}
              {currency}
            </td>
            <th scope="row" className="border border-dark">
              Company Email
      ...
      >
        ...
        <Modal.Footer className="d-flex justify-content-center align-items-center">
          ...
          <Button variant="primary" onClick={handleSaveUserDetails}>
            Save Changes
          </Button>
        </Modal.Footer>
      </Modal>
      <Modal
        ...
        onHide={handleCloseProfessionalModal}
        backdrop="static"
        keyboard={false}
      >
        <Modal.Header closeButton>
        ...
        <Modal.Footer className="d-flex justify-content-center align-items-center">
          <Button variant="secondary" onClick={handleCloseProfessionalModal}>
            Close
          </Button>
          <Button variant="primary" onClick={handleSaveProfessionalDetails}>
            Save Changes
          </Button>
        </Modal.Footer>
      </Modal>
